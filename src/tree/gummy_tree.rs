//! Contains [GummyTree](crate::tree::GummyTree): the default implementation of [LayoutTree](crate::tree::LayoutTree), and the error type for Gummy.
#[cfg(not(feature = "std"))]
...
/// The error Gummy generates on invalid operations
pub type GummyResult<T> = Result<T, GummyError>;
...
pub enum GummyError {
    ...
    /// The parent node was not found in the [`GummyTree`](crate::GummyTree) instance.
    InvalidParentNode(NodeId),
    /// The child node was not found in the [`GummyTree`](crate::GummyTree) instance.
    InvalidChildNode(NodeId),
    /// The supplied node was not found in the [`GummyTree`](crate::GummyTree) instance.
    InvalidInputNode(NodeId),
}

impl core::fmt::Display for GummyError {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            GummyError::ChildIndexOutOfBounds { parent, child_index, child_count } => {
                write!(f, "Index (is {child_index}) should be < child_count ({child_count}) for parent node {parent:?}")
            }
            GummyError::InvalidParentNode(parent) => {
                write!(f, "Parent Node {parent:?} is not in the GummyTree instance")
            }
            GummyError::InvalidChildNode(child) => write!(f, "Child Node {child:?} is not in the GummyTree instance"),
            GummyError::InvalidInputNode(node) => write!(f, "Supplied Node {node:?} is not in the GummyTree instance"),
        }
    }
}

#[cfg(feature = "std")]
impl std::error::Error for GummyError {}

/// Global configuration values for a GummyTree instance
#[derive(Debug, Clone, Copy)]
pub(crate) struct GummyConfig {
...
struct NodeData {
    ...
    /// layout to avoid errors from rounding already-rounded values. See <https://github.com/DioxusLabs/gummy/issues/501>.
...
/// An entire tree of UI nodes. The entry point to Gummy's high-level API.
///
/// Allows you to build a tree of UI nodes, run Gummy's layout algorithms over that tree, and then access the resultant layout.]
#[derive(Debug, Clone)]
pub struct GummyTree<NodeContext = ()> {
...
impl Default for GummyTree {
    fn default() -> GummyTree<()> {
        GummyTree::new()
    }
}

/// Iterator that wraps a slice of nodes, lazily converting them to u64
pub struct GummyTreeChildIter<'a>(core::slice::Iter<'a, NodeId>);
impl Iterator for GummyTreeChildIter<'_> {
    type Item = NodeId;
...
// TraversePartialTree impl for GummyTree
impl<NodeContext> TraversePartialTree for GummyTree<NodeContext> {
    type ChildIter<'a>
        = GummyTreeChildIter<'a>
    where
        Self: 'a;

    #[inline(always)]
    fn child_ids(&self, parent_node_id: NodeId) -> Self::ChildIter<'_> {
        GummyTreeChildIter(self.children[parent_node_id.into()].iter())
    }
...
// TraverseTree impl for GummyTree
impl<NodeContext> TraverseTree for GummyTree<NodeContext> {}

// CacheTree impl for GummyTree
impl<NodeContext> CacheTree for GummyTree<NodeContext> {
...
// PrintTree impl for GummyTree
impl<NodeContext> PrintTree for GummyTree<NodeContext> {
    #[inline(always)]
...
/// View over the Gummy tree that holds the tree itself along with a reference to the context
/// and implements LayoutTree. This allows the context to be stored outside of the GummyTree struct
/// which makes the lifetimes of the context much more flexible.
pub(crate) struct GummyView<'t, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    /// A reference to the GummyTree
    pub(crate) gummy: &'t mut GummyTree<NodeContext>,
...
impl<NodeContext, MeasureFunction> GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    fn compute_child_layout(
        ...
        compute_cached_layout(self, node_id, inputs, |tree, node_id, inputs| {
            let display_mode = tree.gummy.nodes[node_id.into()].style.display;
            ...
            match (display_mode, has_children) {
                ...
                (_, false) => {
                    ...
                    let style = &tree.gummy.nodes[node_key].style;
                    let has_context = tree.gummy.nodes[node_key].has_context;
                    let node_context = has_context.then(|| tree.gummy.node_context_data.get_mut(node_key)).flatten();
...
impl<NodeContext, MeasureFunction> TraversePartialTree for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    #[inline(always)]
    fn child_ids(&self, parent_node_id: NodeId) -> Self::ChildIter<'_> {
        self.gummy.child_ids(parent_node_id)
    }

    #[inline(always)]
    fn child_count(&self, parent_node_id: NodeId) -> usize {
        self.gummy.child_count(parent_node_id)
    }

    #[inline(always)]
    fn get_child_id(&self, parent_node_id: NodeId, child_index: usize) -> NodeId {
        self.gummy.get_child_id(parent_node_id, child_index)
    }
}

// TraverseTree impl for GummyView
impl<NodeContext, MeasureFunction> TraverseTree for GummyView<'_, NodeContext, MeasureFunction> where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>
{
}

// LayoutPartialTree impl for GummyView
impl<NodeContext, MeasureFunction> LayoutPartialTree for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    fn get_core_container_style(&self, node_id: NodeId) -> Self::CoreContainerStyle<'_> {
        &self.gummy.nodes[node_id.into()].style
    }

    #[inline(always)]
    fn set_unrounded_layout(&mut self, node_id: NodeId, layout: &Layout) {
        self.gummy.nodes[node_id.into()].has_new_layout = &self.gummy.nodes[node_id.into()].unrounded_layout != layout;
        self.gummy.nodes[node_id.into()].unrounded_layout = *layout;
    }
...
impl<NodeContext, MeasureFunction> CacheTree for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    fn cache_get(&self, node_id: NodeId, input: &LayoutInput) -> Option<LayoutOutput> {
        self.gummy.nodes[node_id.into()].cache.get(input)
    }

    fn cache_store(&mut self, node_id: NodeId, input: &LayoutInput, layout_output: LayoutOutput) {
        self.gummy.nodes[node_id.into()].cache.store(input, layout_output)
    }

    fn cache_clear(&mut self, node_id: NodeId) {
        self.gummy.nodes[node_id.into()].cache.clear();
    }
}

#[cfg(feature = "block_layout")]
impl<NodeContext, MeasureFunction> LayoutBlockContainer for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
#[cfg(feature = "flexbox")]
impl<NodeContext, MeasureFunction> LayoutFlexboxContainer for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    fn get_flexbox_child_style(&self, child_node_id: NodeId) -> Self::FlexboxItemStyle<'_> {
        &self.gummy.nodes[child_node_id.into()].style
    }
}

#[cfg(feature = "grid")]
impl<NodeContext, MeasureFunction> LayoutGridContainer for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    fn set_detailed_grid_info(&mut self, node_id: NodeId, detailed_grid_info: DetailedGridInfo) {
        self.gummy.nodes[node_id.into()].detailed_layout_info = DetailedLayoutInfo::Grid(Box::new(detailed_grid_info));
    }
}

// RoundTree impl for GummyView
impl<NodeContext, MeasureFunction> RoundTree for GummyView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
    ...
    fn get_unrounded_layout(&self, node: NodeId) -> Layout {
        self.gummy.nodes[node.into()].unrounded_layout
    }

    #[inline(always)]
    fn set_final_layout(&mut self, node_id: NodeId, layout: &Layout) {
        self.gummy.nodes[node_id.into()].final_layout = *layout;
    }
}

#[allow(clippy::iter_cloned_collect)] // due to no-std support, we need to use `iter_cloned` instead of `collect`
impl<NodeContext> GummyTree<NodeContext> {
    /// Creates a new [`GummyTree`]
    ///
    /// The default capacity of a [`GummyTree`] is 16 nodes.
    #[must_use]
    pub fn new() -> Self {
        Self::with_capacity(16)
    }

    /// Creates a new [`GummyTree`] that can store `capacity` nodes before reallocation
    #[must_use]
    ...
    pub fn new_leaf(&mut self, layout: Style) -> GummyResult<NodeId> {
    ...
    pub fn new_leaf_with_context(&mut self, layout: Style, context: NodeContext) -> GummyResult<NodeId> {
    ...
    pub fn new_with_children(&mut self, layout: Style, children: &[NodeId]) -> GummyResult<NodeId> {
    ...
    pub fn remove(&mut self, node: NodeId) -> GummyResult<NodeId> {
        let key = node.into();
    ...
    #[inline]
    pub fn set_node_context(&mut self, node: NodeId, measure: Option<NodeContext>) -> GummyResult<()> {
        let key = node.into();
    ...
    pub fn add_child(&mut self, parent: NodeId, child: NodeId) -> GummyResult<()> {
    ...
    pub fn insert_child_at_index(&mut self, parent: NodeId, child_index: usize, child: NodeId) -> GummyResult<()> {
        let parent_key = parent.into();

        let child_count = self.children[parent_key].len();
        if child_index > child_count {
            return Err(GummyError::ChildIndexOutOfBounds { parent, child_index, child_count });
        }
    ...
    pub fn set_children(&mut self, parent: NodeId, children: &[NodeId]) -> GummyResult<()> {
        let parent_key = parent.into();
    ...
    pub fn remove_child(&mut self, parent: NodeId, child: NodeId) -> GummyResult<NodeId> {
    ...
    ///
    /// The child is not removed from the tree entirely, it is simply no longer attached to its previous parent.
    pub fn remove_child_at_index(&mut self, parent: NodeId, child_index: usize) -> GummyResult<NodeId> {
        let parent_key = parent.into();
        let child_count = self.children[parent_key].len();
        if child_index >= child_count {
            return Err(GummyError::ChildIndexOutOfBounds { parent, child_index, child_count });
    ...
    pub fn remove_children_range<R>(&mut self, parent: NodeId, range: R) -> GummyResult<()>
    where
    ...
    #[inline]
    pub fn child_at_index(&self, parent: NodeId, child_index: usize) -> GummyResult<NodeId> {
        let parent_key = parent.into();
        let child_count = self.children[parent_key].len();
        if child_index >= child_count {
            return Err(GummyError::ChildIndexOutOfBounds { parent, child_index, child_count });
    ...
    pub fn children(&self, parent: NodeId) -> GummyResult<Vec<NodeId>> {
        Ok(self.children[parent.into()].clone())
    }

    /// Sets the [`Style`] of the provided `node`
    #[inline]
    pub fn set_style(&mut self, node: NodeId, style: Style) -> GummyResult<()> {
    ...
    #[inline]
    pub fn style(&self, node: NodeId) -> GummyResult<&Style> {
        Ok(&self.nodes[node.into()].style)
    }

    /// Return this node layout relative to its parent
    #[inline]
    pub fn layout(&self, node: NodeId) -> GummyResult<&Layout> {
        if self.config.use_rounding {
    ...
    pub fn mark_dirty(&mut self, node: NodeId) -> GummyResult<()> {
    ...
    #[inline]
    pub fn dirty(&self, node: NodeId) -> GummyResult<bool> {
            ...
            FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
    {
        ...
        let mut gummy_view = GummyView { gummy: self, measure_function };
        compute_root_layout(&mut gummy_view, node_id, available_space);
        if use_rounding {
            round_layout(&mut gummy_view, node_id);
        }
        Ok(())
    }

    /// Updates the stored layout of the provided `node` and its children
    pub fn compute_layout(&mut self, node: NodeId, available_space: Size<AvailableSpace>) -> Result<(), GummyError> {
    ...
    /// Returns an instance of LayoutTree representing the GummyTree
    #[cfg(test)]
    pub(crate) fn as_layout_tree(&mut self) -> impl LayoutPartialTree + CacheTree + '_ {
        GummyView { gummy: self, measure_function: |_, _, _, _, _| Size::ZERO }
    }
}
...
mod tests {
    ...
    fn new_should_allocate_default_capacity() {
        ...
        let gummy: GummyTree<()> = GummyTree::new();

        assert!(gummy.children.capacity() >= DEFAULT_CAPACITY);
        assert!(gummy.parents.capacity() >= DEFAULT_CAPACITY);
        assert!(gummy.nodes.capacity() >= DEFAULT_CAPACITY);
    }

    #[test]
    fn test_with_capacity() {
        const CAPACITY: usize = 8;
        let gummy: GummyTree<()> = GummyTree::with_capacity(CAPACITY);

        assert!(gummy.children.capacity() >= CAPACITY);
        assert!(gummy.parents.capacity() >= CAPACITY);
        assert!(gummy.nodes.capacity() >= CAPACITY);
    }
    ...
    fn test_new_leaf() {
        ...
        let res = gummy.new_leaf(Style::default());
        assert!(res.is_ok());
        let node = res.unwrap();

        // node should be in the gummy tree and have no children
        assert!(gummy.child_count(node) == 0);
    }

    #[test]
    fn new_leaf_with_context() {
        let mut gummy: GummyTree<Size<f32>> = GummyTree::new();

        let res = gummy.new_leaf_with_context(Style::default(), Size::ZERO);
        assert!(res.is_ok());
        let node = res.unwrap();
    ...
    fn test_new_with_children() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        let child0 = gummy.new_leaf(Style::default()).unwrap();
        let child1 = gummy.new_leaf(Style::default()).unwrap();
        let node = gummy.new_with_children(Style::default(), &[child0, child1]).unwrap();

        // node should have two children
        assert_eq!(gummy.child_count(node), 2);
        assert_eq!(gummy.children(node).unwrap()[0], child0);
        assert_eq!(gummy.children(node).unwrap()[1], child1);
    }
    ...
    fn remove_node_should_remove() {
        ...
        let node = gummy.new_leaf(Style::default()).unwrap();

        let _ = gummy.remove(node).unwrap();
    }
    ...
    fn remove_node_should_detach_hierarchy() {
        ...
        let node2 = gummy.new_leaf(Style::default()).unwrap();
        let node1 = gummy.new_with_children(Style::default(), &[node2]).unwrap();
        let node0 = gummy.new_with_children(Style::default(), &[node1]).unwrap();

        // Both node0 and node1 should have 1 child nodes
        assert_eq!(gummy.children(node0).unwrap().as_slice(), &[node1]);
        assert_eq!(gummy.children(node1).unwrap().as_slice(), &[node2]);

        // Disconnect the tree: <0> <2>
        let _ = gummy.remove(node1).unwrap();

        // Both remaining nodes should have no child nodes
        assert!(gummy.children(node0).unwrap().is_empty());
        assert!(gummy.children(node2).unwrap().is_empty());
    }
    ...
    fn remove_last_node() {
        ...
        let parent = gummy.new_leaf(Style::default()).unwrap();
        let child = gummy.new_leaf(Style::default()).unwrap();
        gummy.add_child(parent, child).unwrap();

        gummy.remove(child).unwrap();
        gummy.remove(parent).unwrap();
    }

    #[test]
    fn set_measure() {
        let mut gummy: GummyTree<Size<f32>> = GummyTree::new();
        let node = gummy.new_leaf_with_context(Style::default(), Size { width: 200.0, height: 200.0 }).unwrap();
        gummy.compute_layout_with_measure(node, Size::MAX_CONTENT, size_measure_function).unwrap();
        assert_eq!(gummy.layout(node).unwrap().size.width, 200.0);

        gummy.set_node_context(node, Some(Size { width: 100.0, height: 100.0 })).unwrap();
        gummy.compute_layout_with_measure(node, Size::MAX_CONTENT, size_measure_function).unwrap();
    ...
    fn set_measure_of_previously_unmeasured_node() {
        ...
        let node = gummy.new_leaf(Style::default()).unwrap();
        gummy.compute_layout_with_measure(node, Size::MAX_CONTENT, size_measure_function).unwrap();
        assert_eq!(gummy.layout(node).unwrap().size.width, 0.0);
    ...
    fn add_child() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        let node = gummy.new_leaf(Style::default()).unwrap();
        assert_eq!(gummy.child_count(node), 0);

        let child0 = gummy.new_leaf(Style::default()).unwrap();
        gummy.add_child(node, child0).unwrap();
        assert_eq!(gummy.child_count(node), 1);

        let child1 = gummy.new_leaf(Style::default()).unwrap();
        gummy.add_child(node, child1).unwrap();
        assert_eq!(gummy.child_count(node), 2);
    }
    ...
    fn insert_child_at_index() {
        ...
        let child0 = gummy.new_leaf(Style::default()).unwrap();
        let child1 = gummy.new_leaf(Style::default()).unwrap();
        let child2 = gummy.new_leaf(Style::default()).unwrap();

        let node = gummy.new_leaf(Style::default()).unwrap();
        assert_eq!(gummy.child_count(node), 0);

        gummy.insert_child_at_index(node, 0, child0).unwrap();
        assert_eq!(gummy.child_count(node), 1);
        assert_eq!(gummy.children(node).unwrap()[0], child0);

        gummy.insert_child_at_index(node, 0, child1).unwrap();
        assert_eq!(gummy.child_count(node), 2);
        assert_eq!(gummy.children(node).unwrap()[0], child1);
        assert_eq!(gummy.children(node).unwrap()[1], child0);

        gummy.insert_child_at_index(node, 1, child2).unwrap();
        assert_eq!(gummy.child_count(node), 3);
        assert_eq!(gummy.children(node).unwrap()[0], child1);
        assert_eq!(gummy.children(node).unwrap()[1], child2);
        assert_eq!(gummy.children(node).unwrap()[2], child0);
    }
    ...
    fn set_children() {
        ...
        let child2 = gummy.new_leaf(Style::default()).unwrap();
        let child3 = gummy.new_leaf(Style::default()).unwrap();
        gummy.set_children(node, &[child2, child3]).unwrap();

        assert_eq!(gummy.child_count(node), 2);
        assert_eq!(gummy.children(node).unwrap()[0], child2);
        assert_eq!(gummy.children(node).unwrap()[1], child3);
    }
    ...
    fn remove_child() {
        ...
        gummy.remove_child(node, child0).unwrap();
        assert_eq!(gummy.child_count(node), 1);
        assert_eq!(gummy.children(node).unwrap()[0], child1);

        gummy.remove_child(node, child1).unwrap();
        assert_eq!(gummy.child_count(node), 0);
    }
    ...
    fn remove_child_at_index() {
        ...
        gummy.remove_child_at_index(node, 0).unwrap();
        assert_eq!(gummy.child_count(node), 1);
        assert_eq!(gummy.children(node).unwrap()[0], child1);
    ...
    fn remove_children_range() {
        ...
        let child2 = gummy.new_leaf(Style::default()).unwrap();
        let child3 = gummy.new_leaf(Style::default()).unwrap();
        let node = gummy.new_with_children(Style::default(), &[child0, child1, child2, child3]).unwrap();

        assert_eq!(gummy.child_count(node), 4);

        gummy.remove_children_range(node, 1..=2).unwrap();
        assert_eq!(gummy.child_count(node), 2);
        assert_eq!(gummy.children(node).unwrap(), [child0, child3]);
        for child in [child0, child3] {
            assert_eq!(gummy.parent(child), Some(node));
        }
        for child in [child1, child2] {
            assert_eq!(gummy.parent(child), None);
        }
    }

    // Related to: https://github.com/DioxusLabs/gummy/issues/510
    #[test]
    fn remove_child_updates_parents() {
        ...
        assert!(gummy.set_children(child, &[]).is_ok());
    }
    ...
    fn replace_child_at_index() {
        ...
        let node = gummy.new_with_children(Style::default(), &[child0]).unwrap();
        assert_eq!(gummy.child_count(node), 1);
        assert_eq!(gummy.children(node).unwrap()[0], child0);

        gummy.replace_child_at_index(node, 0, child1).unwrap();
        assert_eq!(gummy.child_count(node), 1);
        assert_eq!(gummy.children(node).unwrap()[0], child1);
    }
    #[test]
    fn test_child_at_index() {
        ...
        let child1 = gummy.new_leaf(Style::default()).unwrap();
        let child2 = gummy.new_leaf(Style::default()).unwrap();
        let node = gummy.new_with_children(Style::default(), &[child0, child1, child2]).unwrap();

        assert!(if let Ok(result) = gummy.child_at_index(node, 0) { result == child0 } else { false });
        assert!(if let Ok(result) = gummy.child_at_index(node, 1) { result == child1 } else { false });
        assert!(if let Ok(result) = gummy.child_at_index(node, 2) { result == child2 } else { false });
    }
    #[test]
    fn test_child_count() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        ...
        assert!(gummy.child_count(node) == 2);
        assert!(gummy.child_count(child0) == 0);
        assert!(gummy.child_count(child1) == 0);
    }
    ...
    fn test_children() {
        ...
        let children_result = gummy.children(node).unwrap();
        assert_eq!(children_result, children);

        assert!(gummy.children(child0).unwrap().is_empty());
    }
    #[test]
    fn test_set_style() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        ...
        let node = gummy.new_leaf(Style::default()).unwrap();
        assert_eq!(gummy.style(node).unwrap().display, Display::Flex);

        gummy.set_style(node, Style { display: Display::None, ..Style::default() }).unwrap();
        assert_eq!(gummy.style(node).unwrap().display, Display::None);
    }
    #[test]
    fn test_style() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        ...
        let node = gummy.new_leaf(style.clone()).unwrap();

        let res = gummy.style(node);
        assert!(res.is_ok());
    ...
    fn test_mark_dirty() {
        ...
        assert_eq!(gummy.dirty(child0), Ok(false));
        assert_eq!(gummy.dirty(child1), Ok(false));
        assert_eq!(gummy.dirty(node), Ok(false));

        gummy.mark_dirty(node).unwrap();
        assert_eq!(gummy.dirty(child0), Ok(false));
        assert_eq!(gummy.dirty(child1), Ok(false));
        assert_eq!(gummy.dirty(node), Ok(true));

        gummy.compute_layout(node, Size::MAX_CONTENT).unwrap();
        gummy.mark_dirty(child0).unwrap();
        assert_eq!(gummy.dirty(child0), Ok(true));
        assert_eq!(gummy.dirty(child1), Ok(false));
        assert_eq!(gummy.dirty(node), Ok(true));
    }
    ...
    fn set_children_reparents() {
        let mut gummy: GummyTree<()> = GummyTree::new();
        let child = gummy.new_leaf(Style::default()).unwrap();
        let old_parent = gummy.new_with_children(Style::default(), &[child]).unwrap();

        let new_parent = gummy.new_leaf(Style::default()).unwrap();
        gummy.set_children(new_parent, &[child]).unwrap();

        assert!(gummy.children(old_parent).unwrap().is_empty());
    }
}
