...
mod caching {
    ...
    fn measure_count_flexbox() {
        ...
        let leaf = gummy.new_leaf_with_context(Style::default(), NODE_CONTEXT).unwrap();

        let mut node = gummy.new_with_children(Style::DEFAULT, &[leaf]).unwrap();
        for _ in 0..100 {
            node = gummy.new_with_children(Style::DEFAULT, &[node]).unwrap();
        }

        gummy.compute_layout_with_measure(node, Size::MAX_CONTENT, test_measure_function).unwrap();

        assert_eq!(gummy.get_node_context_mut(leaf).unwrap().count, 7);
    }
    ...
    fn measure_count_grid() {
        ...
        let leaf = gummy.new_leaf_with_context(style(), NODE_CONTEXT).unwrap();
