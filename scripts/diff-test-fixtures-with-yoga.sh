...
GUMMY_FIXTURE_DIR=$(SELF=$(dirname "$0") && bash -c "cd \"$SELF\" && cd ../test_fixtures && pwd")
GUMMY_TEST_LIST=$(cd $GUMMY_FIXTURE_DIR && fd -x echo {.} | grep -v '^grid' | sed -E 's/^x(.*)/\1--disabled/' | sort)

diff -U 1000000 --color <(echo "$YOGA_TEST_LIST") <(echo "$GUMMY_TEST_LIST")
